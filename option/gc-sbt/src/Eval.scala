...
object Eval {
  ...
  case object NoneVal extends Value

  // 関数
  case class FValue(xs: List[Var], e: List[Exp])
  ...
  def eval (fenv: Map[Var, FValue], env: Map[Var, Value], e: Exp): Value =
    e match {
      ...
      case ValExp(x, e) => {
        if (env.get(x) == None) NoneVal
        else error(s"$x is already defined")
      }
      case UOpExp(o, e1) => {
      ...
      case IfExp(e, e1, e2) => {
        ...
        v match {
          case BoolVal(true) => bodyeval(fenv, env, e1, NoneVal)
          case BoolVal(false) => bodyeval(fenv, env, e2, NoneVal)
          case _ => error("impossible if the program is well-typed")
        }
      }
      case AppExp(f, es) => {
        ...
        val vs = es.map(eval(fenv, env, _))
                     ...
                     // map を使うと簡単
        bodyeval(fenv, xs.zip(vs).toMap, body, NoneVal)
      }
    }

  def bodyeval(fenv: Map[Var, FValue], env: Map[Var, Value], l: List[Exp], pv: Value): Value = {
    l match {
      case Nil => pv
      case exp::rest => {
        exp match {
          case ValExp(x, e) => {
            val v = eval(fenv, env, e)
            bodyeval(fenv, env+(x->v), rest, NoneVal)
          }
          case _ => {
            val v  = eval(fenv, env, exp)
            bodyeval(fenv, env, rest, v)
          }
        }
      }
    }
