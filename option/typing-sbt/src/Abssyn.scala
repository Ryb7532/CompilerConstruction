...
object Abssyn {
  ...
  case class ValExp(n: Var, b: Exp) extends Exp
  case class UOpExp(o: UOp, e: Exp) extends Exp       // 課題(a)では必要ない
  case class BOpExp(o: BOp, e1: Exp, e2: Exp) extends Exp
  case class IfExp(e: Exp, bd1: List[Exp], bd2: List[Exp]) extends Exp
  case class AppExp(f: Var, es: List[Exp]) extends Exp // 課題(a)では必要ない

  case class Def(name: Var, args: List[(Var,Ty)], rtype: Ty, body: List[Exp])
