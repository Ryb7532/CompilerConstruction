...
class ILTest extends FlatSpec {
  ...
  "if" should "正しい値" in
  {
    assert(evalCV(Map(), Map("x"->ListValue(List()), "y"->IntValue(3)),
      IL.trans(IfExp(UOpExp(IsEmptyOp, VarExp("x")),
        List(BOpExp(PlusOp, IntExp(1), VarExp("y"))),
        List(BOpExp(ConsOp, VarExp("y"), VarExp("x"))))))
      == IntValue(4))
  ...
  "val" should "正しい値" in
  {
    assert(evalCV(Map(), Map(), IL.trans(ValExp("x", IntExp(1)))) == NoneValue)
    assert(evalCV(Map(), Map(), IL.transBody(List(ValExp("x", IntExp(1)), VarExp("x")), IL.NoneVal)) == IntValue(1))
    assert(evalCV(Map(), Map(), IL.transBody(List(ValExp("x", NilExp), VarExp("x")), IL.NoneVal)) == ListValue(List()))
    assert(evalCV(Map(), Map(),
      IL.transBody(
        List(
          ValExp("x", BOpExp(ConsOp, IntExp(1),
            BOpExp(ConsOp, IntExp(2), NilExp))),
          VarExp("x")), IL.NoneVal)) == ListValue(List(1, 2)))
  }

  "examples/append" should "正しい値" in
  {
    val ds = Test.transFileDefs("examples/append.scala")
    val fenv = defs2env(ds)
    assert(
      evalCV(fenv, Map(
        "x"->ListValue(List(1, 2, 3)),
        "y"->ListValue(List(4, 5))),
      IL.trans(AppExp("append", List(VarExp("x"), VarExp("y")))))
      == ListValue(List(1,2,3,4,5)))
  }
  ...
  {
    val ds = Test.transFileDefs("examples/remainder.scala")
    val fenv = defs2env(ds)
    assert(
      evalCV(fenv, Map(
        "x"->IntValue(4),
        "y"->IntValue(2)),
      IL.trans(AppExp("test", List(VarExp("x"), VarExp("y")))))
      == IntValue(0))
    assert(
      evalCV(fenv, Map(
        "x"->IntValue(5),
