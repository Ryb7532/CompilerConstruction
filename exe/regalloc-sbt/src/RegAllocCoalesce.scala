...
object RegAllocCoalesce {
  ...
  def canCoalesce (g: Graph[Reg], k: Int, r1: Reg, r2: Reg): Boolean = {
    if (r1 == r2) false
    else if (g.edges.contains((r1, r2))) false
    else if (!allRegs.contains(r1) && !allRegs.contains(r2)) {
      val g1 = g.merge(r1, r2)
      val a = g1.adjacent(r1).filter(deg(g1, _) >= k)
      if (a.size < k) true
      else false
    } else if (allRegs.contains(r1) && !allRegs.contains(r2)) {
      val a = g.adjacent(r2).filter(!g.adjacent(r1).contains(_))
      val b = a.filter(deg(g, _) >= k)
      val c = b.filter(!allRegs.contains(_))
      if (c.size > 0) false
      else true
    } else false
  }
