...
class EvalTest extends FlatSpec {
  ...
  {
    assert(
      eval(Map(), Map("x"->BoolVal(true)), IfExp(VarExp("x"), IntExp(1), IntExp(0)))
      == IntVal(1))
    assert(
      eval(Map(), Map("x"->BoolVal(false)), IfExp(VarExp("x"), IntExp(1), IntExp(0)))
      == IntVal(0))

    assertThrows[EvalError] {
      eval(Map(), Map("x"->IntVal(1)), IfExp(VarExp("x"), IntExp(2), IntExp(3)))
    }
    assertThrows[EvalError] {
      eval(Map(), Map("x"->ListVal(List(0))), IfExp(VarExp("x"), IntExp(2), IntExp(3)))
    }
  }
